...
class UiDataService {
    ...
    if (client != null) {
      try {
        ...
        final filters = [
          'user_id.eq.$userId',
          'target_role.eq.all',
          if (role != null) 'target_role.eq.$role',
        ].join(',');
        final data = await client
            .from('notifications')
            .select()
    ...
    if (client != null) {
      try {
        final data = await client
            .from('support_tickets')
            .select(
                'id, ticket_code, subject, description, status, priority, created_at, updated_at')
            .eq('user_id', userId)
            .order('created_at', ascending: false);
        return (data as List)
  ...
  Future<String?> saveMedicalDocumentMeta({
    ...
    try {
      final data = await client
          ...
          .insert({
            ...
            'storage_bucket': bucket ?? 'medical-documents',
  ...
  Future<List<Map<String, dynamic>>> getMedicalDocuments(
    ...
    if (client != null) {
      try {
        final data = await client
            .from('medical_documents')
            .select(
                'id, file_name, document_type, storage_path, storage_bucket, status, created_at')
