...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  /// Step 1 – Initialize payment with Paystack and open the hosted checkout page.
  Future<void> _initPayment() async {
    ...
    try {
      ...
      final authorizationUrl = initialization['authorization_url']?.toString();

      if (reference == null || reference.isEmpty || authorizationUrl == null) {
        throw 'Payment initialization failed – no reference returned';
      }
  ...
  /// Step 2 – Verify the transaction after the user returns from Paystack.
  Future<void> _verifyPayment() async {
    final user = _authService.currentUser;
    if (user == null || _paystackReference == null) return;

    setState(() => isVerifying = true);
    try {
      final verified =
          await _paymentRepository.verifyPayment(_paystackReference!);
      if (!verified) {
        throw 'Payment not confirmed yet. Please wait a moment and try again, '
            'or contact support if payment was deducted.';
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Payment confirmed! Your appointment is booked.'),
  ...
  Widget _buildConfirmButton(double fee) {
    final busy = isProcessing || isVerifying;

    // After the URL has been opened, show a two-button layout:
    // [Open payment page again]  [I've paid – verify]
    if (paymentUrlOpened) {
      return Column(
        children: [
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: busy ? null : _verifyPayment,
          ...
          TextButton(
            onPressed: busy ? null : _initPayment,
            child: Text(
              'Reopen payment page',
    ...
    return SizedBox(
      ...
      height: 54,
      child: ElevatedButton(
        onPressed: isProcessing ? null : _initPayment,
        style: ElevatedButton.styleFrom(
        ...
        child: isProcessing
            ...
            : Row(
                ...
                children: [
                  ...
                  Text(
                    'Pay GHS ${fee.toStringAsFixed(2)} via Paystack',
                    style: const TextStyle(
                      fontSize: 16,
                      fontWeight: FontWeight.bold,
                      color: Colors.white,
